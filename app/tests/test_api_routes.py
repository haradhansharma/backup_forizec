...
API_ROUTES = [
    ...
    ("POST", "/api/v1/user/login", 200, "application/json", ["access_token", "token_type"], {"username": "testuser", "password": "secret"}, None),
    ("POST", "/api/v1/users", 201, "application/json", ["id", "username", "email"], {"username": "newuser", "email": "new@example.com", "password": "secret"}, None),
    ("PUT", "/api/v1/users/1", 200, "application/json", ["id", "username", "email"], {"username": "updateduser"}, {"Authorization": "Bearer faketoken"}),
    ("PATCH", "/api/v1/users/1", 200, "application/json", ["id", "username"], {"username": "patcheduser"}, {"Authorization": "Bearer faketoken"}),
...
async def test_api_routes(client, method, route, expected_status, expected_content_type, expected_contents, payload, headers):
